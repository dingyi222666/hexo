...
当然，我们今天不是来学习怎么写 lexer 的，只是科普一下这个东西。使用 lexer 我们就可以把下面这段代码
...
形如这样的 token 流。眼尖你的一定发现了 `STRING`！没错，词法分析可以扫描出代码里的字符串，这样我们就能处理代码里的字符串了。

并且 lexer 相比正则表达式，它更简单易用（在调用库的情况下）。并且严格遵循语言的语法规范进行扫描，不会像正则表达式，匹配一些复杂的代码可能会出问题。

现在我们知道了如何匹配并处理字符，但我们还不知道如何处理字符串，来达到一个混淆的效果。

在 lua 5.3 中，新增了 `utf8` 模块，可以使用 `utf8.char` 来将一个 unicode 字符码转换成对应的 utf8 字符。

这个方法允许传入多个 unicode 字符码，返回一个 utf8 字符，所以我们可以直接把字符串转换成 unicode 字符码，再1调用该方法来获取原字符串。

基础的思路都有了，我们就可以开始编写实现代码了。

## 最初实现

### 引入库

打开 Intellij IDEA, 新建一个 kotlin 项目，引入我上面的 Lua 解析库：
...
这里的 `buffer` 是用于存储 `lexer` 扫描出的 token 对应代码的，等会我们在实现混淆的时候这个会有大用处。
...
上面的 `parseLuaString` 将 lexer 扫描出的纯字符串进行转义，转换成实际字符串值，如 `hello world` 会被转换为 `???`。

修改一下 `stringFog` 方法，将 `LuaTokenTypes.STRING` 或者 `LuaTokenTypes.LONG_STRING` 的处理方式改为调用 `getObfuscationString` 处理。
...
可以看到，我们的字符串已经被简单的混淆了。

### 优化实现，支持长字符串

然而对于大字符串，因为 `utf8.char` 能传入的参数量有限（变长参数，大约在 256 个左右，这是 Lua 语言本身的设计限制）。如果一股脑的全部传入，会导致运行时出现错误。我们需要对长字符串进行额外的处理。

我们可以对字符串进行分割，再将分割后的字符串进行处理，最后合并起来。
...
这样就同时支持短字符串和长字符串；，肉眼上看上去也有不错的混淆效果。

## 优化实现，使用常量池

但上面的代码出现了一堆 `utf8.char` 和其参数，很明显这太多了，显然会拖慢程序的运行效率。我们可以考虑用常量池，简化输出并且复用重复的常量数字，下面是一些思路：

由于每个字符串都可以被转换为 unicode 数组，我们可以考虑把字符串转换为 unicode 数组，将这些数组的数字存储到常量池中，最后在代码中引用这些常量数字。
...
为什么不直接在 `string_pool` 中存储字符串呢？这是为了防止直接通过 `print(string_pool)` 拿到整个字符串数组避免直接的还原。后面也会尝试对这些数字进行加密，增强混淆强度。

OK,我们就可以编写一个 `StringPool` 类，写出下面的代码：
...
好像还不错！但是现在的常量依旧不够安全，每个表里面都能被直接转换成字符串，我们还需要再加一层保护。

## 增加常量加密

我们可以再对常量做一层加密：执行混淆时输出加密后的结果，在运行时重新解密，就能提高常量数字的安全性了。
...
无需修改 `stringFog` 函数，重新运行一下，看看效果如何：
...
现在的字符串混淆只对字符串混淆，但是我们都知道在 lua 里，以下几种代码互相等价：

1. 形如 `a.b = c` 的代码等价于 `a['b'] = c`。（因为是表调用）
2. 形如 `a.b(c)` 的代码等价于 `a['b'](c)`。（因为是表调用）
3. 全局变量存于 `_G` 表，因此基于第一点和第二点, `print('h')` 也等价于 `_G['print']('h')`

发现了吗？这样可以让我们制造出更多的字符串，就可以实现混淆隐藏部分字段，让代码逻辑看起来更加的混乱。
...
完成！一个支持字符串加密，并且附带一点点字符隐藏建议的字符串混淆就完成啦。
